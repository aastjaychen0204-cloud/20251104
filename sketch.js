...
function mouseClicked() {
	...
	if (menuX > -menuWidth + 10) {
		...
		for (let i = 0; i < menuItems.length; i++) {
			...
			// 檢查滑鼠是否點擊在當前項目範圍內
			if (mouseX > menuX && mouseX < menuX + menuWidth && mouseY > itemY && mouseY < itemY + itemHeight) {
				// 如果該項目有 URL，則創建 iframe
				if (item.url) {
...
function createIframe(url) {
	...
	iframeElement = createElement('iframe', url);
	iframeElement.position(width * 0.1, height * 0.1);
	iframeElement.size(width * 0.8, height * 0.8);
	iframeElement.style('border', '2px solid #333');

	closeButton = createButton('X');
	closeButton.position(width * 0.9 - 30, height * 0.1 + 5);
	closeButton.style('background-color', '#f71735');
	closeButton.style('color', 'white');
	closeButton.style('border', 'none');
	closeButton.style('font-size', '16px');
	closeButton.style('cursor', 'pointer');
	closeButton.mousePressed(closeIframe);
}
